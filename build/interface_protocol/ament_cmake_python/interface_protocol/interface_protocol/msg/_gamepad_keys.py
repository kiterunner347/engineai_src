...
class GamepadKeys(metaclass=Metaclass_GamepadKeys):
    ...
    def __init__(self, **kwargs):
        ...
        if 'digital_states' not in kwargs:
            ...
            self.digital_states = numpy.array(kwargs.get('digital_states'), dtype=numpy.int32)
            assert self.digital_states.shape == (12, )
        if 'analog_states' not in kwargs:
            self.analog_states = numpy.zeros(6, dtype=numpy.float64)
        else:
            self.analog_states = numpy.array(kwargs.get('analog_states'), dtype=numpy.float64)
            assert self.analog_states.shape == (6, )
